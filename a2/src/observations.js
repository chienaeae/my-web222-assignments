...
function speciesCoordinates(data) {
  ...
  for (let i = 0; i < data.per_page; i++) {
    ...
    const speciesGuess = element.species_guess;
...
function speciesCoordinates2(data) {
  ...
  // console.log(`"Muskrat" observed at coordinates (${coordinates[0]},${coordinates[1]})`);
}
...
function speciesCoordinates3(data) {
  ...
  for (const element of data.results) {
    const speciesGuess = element.species_guess;
    const coordinates = element.geojson.coordinates;
    console.log(`"${speciesGuess}" observed at coordinates (${coordinates[0]}, ${coordinates[1]})`);
  }
}
...
function observationsByQualityGrades(data, ...qualityGrades) {
  ...
  qualityGrades.forEach((grade) => {
    result = result.concat(observationsByQualityGrade(data, grade));
  });
  return result;
}

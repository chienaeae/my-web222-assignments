...
function toCamelCase(name, uppercase) {
  let words = name.split(/[^a-zA-Z0-9]/);
...
function formatTimes(...values) {
  // Replace this comment with your code...
  const normalizeTimes = values.map((value) => normalizeTime(value)).filter(Boolean);
...
function buildApiEndpoint(resourceType, resourceId, extraData) {
  ...
  const q = encodeURIComponent(extraData);
  return `/${resourceType}/${resourceId}${q !== 'undefined' ? `/${q}` : ''}`;
}

...
function renderArtistsBar() {
    ...
    artists.forEach((artist, i, self)=>{
        ...
        if (artist.artistId === chosenArtist.artistId) buttonElement.disabled = true;
        else {
...
function renderSongTable() {
    ...
    const linksHTML = chosenArtist.urls.map((url)=>`<a href="${url.url}">${url.name}</a>`).join(", ");
    ...
    // render song table
    const filteredSongs = songs.filter((song)=>song.artistId === chosenArtist.artistId && !song.explicit);

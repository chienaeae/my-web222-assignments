/**
 * This is a customized JS taken from WIRISeditor-7.16.1 (War provided by WIRIS). We bypass the default one and use
 * this instead. If bumping Wiris version, remember to update this file and include customizations as well.
 */
(function(){
...
/** BB Code Begin - Store ShowImg and Service requests results on memory to avoid calling the services, based on the generated digest. **/
var showImgCache = {};
var serviceCache = {};

// Throttle async requests semaphore.
var showImgThrottle = {};
/** BB Code End. **/
...
com.wiris.js.JsPluginViewer.prototype = {
	...
	,callService: function(mml,servicename) {
		...
		/** BB Code Start Service Call cache **/
		var cacheKey = haxe.Md5.encode(mml + servicename);

		if ( typeof serviceCache[cacheKey] === 'undefined' )
		{
			con.send(data);
			serviceCache[cacheKey] = con.responseText;
			return con.responseText;
		}
		else
		{
	...
	,	callShowimage: function(container,mml,img,asynchronously,callbackFunc) {
		...
		var onloadFunc = function(e) {
			var getResultFunc = function(e1) {
				...
				/** BB Code Begin - Cache service call result storage. **/
				if ( !e1.bbCached )
				{
					try
					{
						...
						if ( typeof result !== 'undefined')
						{
							...
							// Store request result on memory cache.
							showImgCache[md5] = result;
						}
					}
					catch ( e2 )
					{
				...
				{
					result = showImgCache[md5];
					e1 = result["bbE1"];

					// Replace target image container by the new one.
					e1.target.wiris.img = img;
				}

				delete showImgThrottle[md5];
				/** BB Code End. **/
				...
				if(result.alt != null) e1.target.wiris.img.alt = result.alt; else {
					...
					if(haxe.Json.parse(accessibilityResponse).status != "error")
					{
						...
						/** BB Code Begin - Update nested accesible call with result to the cache entry. **/
						if ( typeof showImgCache[md5] !== 'undefined' )
						{
							showImgCache[md5].alt = e1.target.wiris.img.alt;
						}
						/** BB Code End. **/
					}
				}
			...
			/** BB Code Begin - Call showImg POST if not cached. **/
			...
			if ( !e.bbCached )
			{
				if ( haxe.Json.parse( con.responseText ).status == "warning" )
				{
					...
					if ( new com.wiris.js.JsBrowser().isIE() ) data = "centerbaseline=false&mml=" + StringTools.urlEncode(
						e.target.wiris.mml ) + "&useragent=IE"; else data = "centerbaseline=false&mml=" + StringTools.urlEncode(
						e.target.wiris.mml );
					var newDynamic = {};
					con.wiris = newDynamic;
					con.wiris.callbackFunc = callbackFunc;
					...
					{
						if ( com.wiris.js.JsPluginViewer.DEBUG ) haxe.Log.trace( "ShowImage call failed!", {
		...
		/** BB Code Start handle showImg cache call **/
		if ( typeof showImgCache[md5] === 'undefined' )
		{
			// Throttle async requests for the same digest (handle request where its cache is still not populated),
			// as we want to get them done after the first service call is requested to the server, so we rely on the cache.
			if ( typeof showImgThrottle[md5] === 'undefined' )
			{
				showImgThrottle[ md5 ] = container;
				con.send( null );
			}
			else
			{
				...
				// Wait until cache of an ongoing request is populated.
				var throttleCheck =  function()
				{
					timeout++;
					if ( typeof showImgThrottle[md5] !== 'undefined' && timeout <= 100 )
					{
						setTimeout( throttleCheck, 50 );
					}
					else if ( typeof showImgCache[md5] !== 'undefined' )
					{
						// Cache entry was created in the meantime, call the onload function.
						onloadFunc( {bbCached: true} );
					}
					else if ( timeout > 100 )
					{
						// If the throttle is not removed or no caching happened in 5 seconds for some reason, remove throttle entry and call the service.
						delete showImgThrottle[md5];
						con.send( null );
					}
				}
...
haxe.Md5.encode = function(s) {
	...
	// and tiny_mce_wiris/plugin.js (lines 1033 - 1056, 314 - 339).
	// Please find customizations by searching with "BB Code Start".
	function decodeHtmlCharCodes(str) {
		str = str.replace(/(&(\w+);)|(&#x([\da-fA-F]+);)/g, function(match, matchGroup1, matchGroup2, matchGroup3, matchGroup4) {
	...
	let digest = decodeHtmlCharCodes( s );
